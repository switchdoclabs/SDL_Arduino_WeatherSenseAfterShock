...
class D7SClass
{
  ...
  public:
    ...
    float getTempDataLast(int element);
    
    float getSIDataGreatest(int element);
    float getPGADataGreatest(int element);
    float getTempDataGreatest(int element);

    void zeroClear();
    void clearEarthquakeData();
    void clearInstallationData();
    void clearLastestOffsetData();
    void clearSelftestData();
    void clearAllData();
    ...
    void setThreshold(d7s_threshold thresholdMode);

    //get Instantaneous SI (during an earthquake) [m/s]
    float getInstantaneousSI();
    //get Instantaneous PGA (during an earthquake) [m/s^2]
    float getInstantaneousPGA();
